...
class Preprocessor(object):
    ...
    def process_kps(self, kps: np.ndarray, visibility: np.ndarray):
        # Deal with the image patch
        kps_tensor = kps.cuda().unsqueeze(dim=0)  # (1,H,W)
        # Deal with the attention mask
...
class Tracker():
	...
	def load_model(self, template_path="template.pt", search_path="complete.pt"):
			
		self.model_t = torch.jit.load(template_path).cuda().eval()
		# self.model_t = self.model_t.cuda()
		# self.model_t.eval()

		self.model_s = torch.jit.load(search_path).cuda().eval()
	...
	def init_model(self, image, info_: dict):
		...
		# fig, ax = plt.subplots()
		# ax.imshow(z_patch_arr)
		# for kp in range(5):
		# 	x, y = new_kps[kp]
		# 	print(x.item(),y.item(), size)
		# 	circle = plt.Circle((x.item() * size, y.item() * size), 10, color=colors[kp])
		# 	ax.add_patch(circle)
		# plt.show()
	...
	def run_model(self, image, info_):
		...
		# size = 320
		# fig, ax = plt.subplots()
		# ax.imshow(x_patch_arr)
		# for kp in range(5):
		# 	x, y = pred_joints_[kp]
		# 	print(x,y)
		# 	circle = plt.Circle((x * size, y * size), 15, color=colors[kp])
		# 	ax.add_patch(circle)
		# plt.show()
