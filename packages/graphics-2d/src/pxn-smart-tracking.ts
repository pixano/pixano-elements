...
export class SmartTracking extends Rectangle {
	...
	constructor() {
		...
		// controller definition / modification
		this.handleTrackSelection();

		// events definition / mofication
	...
	protected trackTillNextFrame() {
		...
		// this.targetShapes.forEach((s: ShapeData) => {
		// 	// get the shape to track
		// 	console.log("type=",s.geometry.type)
		// 	const v: number[] = s.geometry.vertices;
		// 	const xmin = Math.min(v[0], v[2]);
		// 	const xmax = Math.max(v[0], v[2]);
		// 	const ymin = Math.min(v[1], v[3]);
		// 	const ymax = Math.max(v[1], v[3]);
