...
export class PxnRenderer extends PIXI.Application {
    ...
    public normalizeX(x: number) {
        // return (x  - this.rx  - this.sx) / (this.rw * this.s);
    ...
    get canvasWidth() {
      try {
        return (<HTMLDivElement>this.view.parentNode).offsetWidth
      } catch {
        return 800;
      }
    }
    ...
    get canvasHeight() {
      try {
        return (<HTMLDivElement>this.view.parentNode).offsetHeight
      } catch {
        return 800;
      }
    }
    ...
    public resizeWindow() {
      ...
      // this.view.style.width = this.canvasWidth + 'px';
      // this.view.style.height = this.canvasHeight + 'px';
    }
    ...
    public bringToFront(el: PIXI.Container) { 
      let arr = this.labelLayer.children;
